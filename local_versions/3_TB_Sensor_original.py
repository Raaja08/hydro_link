...
USE_GOOGLE_DRIVE = st.sidebar.checkbox("📁 Use Google Drive Data", value=False, help="Check to load data from Google Drive instead of local storage")
...
st.set_page_config(page_title="TB Sensor (Rainfall & Temperature) Viewer", layout="wide")
...
if USE_GOOGLE_DRIVE and GOOGLE_DRIVE_ENABLED:
    ...
    # Local file selection (original logic)
...
if selected_file:
    ...
    if plot_df.empty:
        ...
        if view_mode in ["Daily", "Monthly", "Yearly"] and st.button(f"⬇️ Download All {view_mode} Plots as ZIP"):
            with tempfile.TemporaryDirectory() as tmpdir:
                zip_path = os.path.join(tmpdir, f"{sensor_id}_{view_mode}_plots.zip")
                
                with zipfile.ZipFile(zip_path, 'w') as zipf:
                    if view_mode == "Daily":
                        # Use the current aggregation setting for all daily plots
                        current_agg = "Hourly"  # Default
                        if 'aggregation' in locals():
                            current_agg = agg
                        
                        freq = "15min" if current_agg == "15-min" else "H"
                        interval_text = "15 min interval" if current_agg == "15-min" else "one hour interval"
                        
                        daily_bins = df.groupby(df.index.date).size().index
                        daily_bins = [pd.Timestamp(date) for date in daily_bins]
                        
                        for date_bin in daily_bins:
                            daily_filtered = df[(df.index >= date_bin) & (df.index < date_bin + timedelta(days=1))]
                            if not daily_filtered.empty:
                                daily_range = pd.date_range(start=date_bin, end=date_bin + pd.Timedelta(days=1), freq=freq, inclusive='left')
                                daily_plot_df = daily_filtered.resample(freq).agg({
                                    'rainfall_mm': lambda x: x.sum() if (len(x) > 0 and x.notna().any()) else None
                                }).reindex(daily_range)
                                
                                daily_fig = px.bar(daily_plot_df, y="rainfall_mm", 
                                                 title=f"Rainfall (mm) (Daily: {date_bin.strftime('%B %d, %Y')} - {interval_text})",
                                                 template="plotly_white", color_discrete_sequence=["#1f77b4"])
                                
                                # Set y-axis range
                                max_val = daily_plot_df['rainfall_mm'].max()
                                if pd.isna(max_val) or max_val == 0:
                                    max_val = 1
                                daily_fig.update_layout(
                                    yaxis=dict(title="Rainfall (mm)", showgrid=True, gridwidth=1, 
                                             gridcolor='lightgray', range=[0, max_val * 1.2]),
                                    xaxis_title="Time", height=400
                                )
                                
                                img_path = os.path.join(tmpdir, f"{sensor_id}_daily_{date_bin.strftime('%Y%m%d')}.png")
                                pio.write_image(daily_fig, img_path, scale=3, width=1200, height=600)
                                zipf.write(img_path, f"{sensor_id}_daily_{date_bin.strftime('%Y%m%d')}.png")
                    
                    elif view_mode == "Monthly":
                        monthly_groups = df.groupby(pd.Grouper(freq='MS')).size()
                        monthly_bins = monthly_groups[monthly_groups > 0].index
                        
                        for month_bin in monthly_bins:
                            month_end = month_bin + pd.offsets.MonthEnd(1)
                            monthly_filtered = df[(df.index >= month_bin) & (df.index < month_end)]
                            if not monthly_filtered.empty:
                                monthly_range = pd.date_range(start=month_bin, end=month_end, freq='D')
                                monthly_plot_df = monthly_filtered.resample('D').agg({
                                    'rainfall_mm': lambda x: x.sum() if (len(x) > 0 and x.notna().any()) else None
                                }).reindex(monthly_range)
                                monthly_plot_df['cumulative_rainfall'] = monthly_plot_df['rainfall_mm'].cumsum()
                                
                                monthly_fig = px.bar(monthly_plot_df, y="rainfall_mm", 
                                                   title=f"Rainfall (mm) (Monthly: {month_bin.strftime('%B %Y')})",
                                                   template="plotly_white", color_discrete_sequence=["#1f77b4"])
                                
                                # Add cumulative line
                                monthly_fig.add_scatter(x=monthly_plot_df.index, y=monthly_plot_df['cumulative_rainfall'], 
                                                      mode="lines+markers", name="Cumulative Rainfall", 
                                                      line=dict(color="#00509E"), yaxis="y2", connectgaps=False)
                                
                                # Set y-axis ranges
                                max_rainfall = monthly_plot_df['rainfall_mm'].max()
                                max_cumulative = monthly_plot_df['cumulative_rainfall'].max()
                                if pd.isna(max_rainfall) or max_rainfall == 0:
                                    max_rainfall = 1
                                if pd.isna(max_cumulative) or max_cumulative == 0:
                                    max_cumulative = 1
                                
                                monthly_fig.update_layout(
                                    yaxis=dict(title="Rainfall (mm)", side="left", showgrid=True, 
                                             gridwidth=1, gridcolor='lightgray', range=[0, max_rainfall * 1.2]),
                                    yaxis2=dict(title="Cumulative Rainfall (mm)", side="right", 
                                              overlaying="y", showgrid=False, range=[0, max_cumulative * 1.2]),
                                    xaxis_title="Time", height=400, hovermode='x unified'
                                )
                                
                                img_path = os.path.join(tmpdir, f"{sensor_id}_monthly_{month_bin.strftime('%Y%m')}.png")
                                pio.write_image(monthly_fig, img_path, scale=3, width=1200, height=600)
                                zipf.write(img_path, f"{sensor_id}_monthly_{month_bin.strftime('%Y%m')}.png")
                    
                    elif view_mode == "Yearly":
                        yearly_groups = df.groupby(pd.Grouper(freq='YS')).size()
                        yearly_bins = yearly_groups[yearly_groups > 0].index
                        
                        for year_bin in yearly_bins:
                            year_end = year_bin.replace(month=12, day=31)
                            yearly_filtered = df[(df.index >= year_bin) & (df.index <= year_end)]
                            if not yearly_filtered.empty:
                                yearly_range = pd.date_range(start=year_bin, end=year_end, freq='MS')
                                yearly_plot_df = yearly_filtered.resample('MS').agg({
                                    'rainfall_mm': lambda x: x.sum() if (len(x) > 0 and x.notna().any()) else None
                                }).reindex(yearly_range)
                                yearly_plot_df['cumulative_rainfall'] = yearly_plot_df['rainfall_mm'].cumsum()
                                
                                yearly_fig = px.bar(yearly_plot_df, y="rainfall_mm", 
                                                  title=f"Rainfall (mm) (Yearly: {year_bin.strftime('%Y')})",
                                                  template="plotly_white", color_discrete_sequence=["#1f77b4"])
                                
                                # Add cumulative line
                                yearly_fig.add_scatter(x=yearly_plot_df.index, y=yearly_plot_df['cumulative_rainfall'], 
                                                     mode="lines+markers", name="Cumulative Rainfall", 
                                                     line=dict(color="#00509E"), yaxis="y2", connectgaps=False)
                                
                                # Set y-axis ranges
                                max_rainfall = yearly_plot_df['rainfall_mm'].max()
                                max_cumulative = yearly_plot_df['cumulative_rainfall'].max()
                                if pd.isna(max_rainfall) or max_rainfall == 0:
                                    max_rainfall = 1
                                if pd.isna(max_cumulative) or max_cumulative == 0:
                                    max_cumulative = 1
                                ...
                                img_path = os.path.join(tmpdir, f"{sensor_id}_yearly_{year_bin.strftime('%Y')}.png")
                                pio.write_image(yearly_fig, img_path, scale=3, width=1200, height=600)
                                zipf.write(img_path, f"{sensor_id}_yearly_{year_bin.strftime('%Y')}.png")
                
                with open(zip_path, "rb") as f:
                    st.download_button(
                        f"📦 Download {view_mode} Plots ZIP",
                        f.read(),
                        file_name=f"{sensor_id}_{view_mode}_plots.zip",
