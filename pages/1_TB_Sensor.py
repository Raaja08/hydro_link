# TB Sensor - Simplified GitHub Version
...
# TB Sensor parameters - updated to match actual CSV columns
AVAILABLE_PARAMS = {
...
if logo_base64:
    st.markdown(f"""
        <div style='display: flex; justify-content: space-between; align-items: center; padding: 20px 10px 10px 10px;'>
            <div>
                <h1 style='margin-bottom: 0;'>🌊 S4W Sensor Dashboard - TB Sensor</h1>
...
try:
    ...
    if not sites:
        st.error("🔍 No sensor sites found in the processed/tb folder.")
        st.stop()
...
if selected_file:
    ...
    # Get sensor metadata
    sensor_id = selected_file.split(".")[0]  # e.g., tb_s1_2023
    ...
    selected_params = [p for p in AVAILABLE_PARAMS.keys() if p in df.columns and st.sidebar.checkbox(AVAILABLE_PARAMS[p], p == 'rainfall_mm')]
